...
const STATES_DIR = path.resolve(__dirname, '..', '..', 'data', 'seed', 'states');
...
function main(): void {
  console.log('=== US Law MCP — State Statute Ingestion ===');
  ...
  const stateFiles = fs.readdirSync(STATES_DIR).filter(f => f.endsWith('.json')).sort();
  ...
  const ingestAll = db.transaction(() => {
    for (const file of stateFiles) {
      ...
      const state = path.basename(file, '.json').toUpperCase();

      console.log(`  Processing ${state} (${file})...`);
  ...
  console.log(`  Total: ${totalDocs} documents, ${totalProvs} provisions across ${stateFiles.length} states`);
